...
def create_buttons():
    ...
    # Button to retrieve and display saved attempts
    retrieve_button = tk.Button (racine , text = "Retrieve Attempts" , width = 15 , command = display_attempts)
    retrieve_button.grid (row = len (couleurs) + 1 , column=4)

    hint_button = tk.Button (racine , text = "Hint" , width = 15 , command = provide_hint)
    hint_button.grid (row = len (couleurs) + 1 , column = 4)
...
def save_attempts(attempts, filename="attempts.txt"):
    with open(filename, "a") as file:
        for attempt in attempts:
            file.write(str(attempt) + "\n")


# function to read attempts:
def read_attempts(filename="attempts.txt"):
    attempts = []
    with open(filename, "r") as file:
        for line in file:
            attempts.append(eval(line.strip()))
    return attempts
...
def display_attempts():
    ...
    affiche_texte(attempts_text)

#function provide hint
def provide_hint():
    global code_secret, revealed_hints
    if len(revealed_hints) < 4:
        hint_index = len(revealed_hints)
        hint_color = code_secret[hint_index]
        revealed_hints.append(hint_color)
        hint_text = f"Hint {hint_index + 1}: Color at position {hint_index + 1} is {hint_color}"
        affiche_texte(hint_text)
    else:
        affiche_texte("No more hints available!")
...
def valider():
    '''Validate the player's guess and provide feedback'''
    ...
    if current_circle == 4:
        ...
        if correct_positions == 4:
            ...
            GAGNE = True
            end_game()
            save_attempts(tentatives)  # Save attempts when the game ends
...
def end_game():  # j'ai eu un peu d'aide pour celui la.
    ...
    rejouer_button = tk.Button (button_frame , text = "Rejouer" , width = 10 ,
                                command = lambda: [dialog.destroy () , nouvelle_partie ()])
    rejouer_button.pack (side = "left" , padx = 10)

    retour_button = tk.Button (button_frame , text = "Menu principal" , width = 15 ,
                               command = lambda: [dialog.destroy () , retour_main_menu ()])
